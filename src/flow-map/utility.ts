...
function button(
  ...
  this.state =
    typeof state != "undefined"
      ? state ? CVS_BUTTON_DOWN : CVS_BUTTON_UP
      : CVS_BUTTON_UP;
  ...
  this.eventhandler = function(x, y, e) {
    ...
    if (e.type == "keydown") {
      if (e.keyCode == 13) {
        // <ENTER> key
        if (!self.keyupID) {
          self.keyupID = self.eventmanager.listen(
            "keyup",
            self.hit,
            self.eventhandler
          );
        ...
        if (self.toggle) {
          self.state =
            self.state == CVS_BUTTON_DOWN ? CVS_BUTTON_UP : CVS_BUTTON_DOWN;
        } else {
      ...
      if (!self.mouseupID && !self.toggle) {
        self.mouseupID = self.eventmanager.listen(
          "mouseup",
          self.hit,
          self.eventhandler
        );
      ...
      if (!self.mouseclickID && !self.toggle) {
        self.mouseclickID = self.eventmanager.listen(
          "mouseclick",
          self.hit,
          self.eventhandler
        );
      ...
      if (!self.mouseoutID && !self.toggle) {
        ...
        // listening for stuff
        self.mouseoutID = self.eventmanager.listen(
          "mouseout",
          self.hit,
          self.eventhandler
        );
  ...
  this.draw = function() {
    ...
    if (self.type == CVS_BUTTON_NORMAL) {
      ...
      if (self.state == CVS_BUTTON_DOWN) {
        self.ctx.fillStyle = clrObj.mult(0.75);
        ctx.roundedRect(
          self.x + 1,
          self.y + 1,
          self.width - 3,
        ...
        var grad = ctx.createLinearGradient(
          ...
          self.y + 1,
          self.x + 1,
          self.height - 3 + self.y
        );
        grad.addColorStop(0, "rgba(180,180,180,.6)");
        grad.addColorStop(0.3, "rgba(245,245,245,1.0)");
        ...
        self.ctx.fillStyle = clrObj.mult(0.92);
        ctx.roundedRect(
          self.x + 2,
          self.y + 2,
          self.width - 4,
    ...
    if (self.name) {
      ...
      var y =
        self.toggle || self.state == CVS_BUTTON_UP
          ? self.y + self.height / 2
          : self.y + self.height / 2 + 1;
      var x = self.toggle
        ? self.x + 0.25 * self.width + 20
        : self.x + self.width / 2 - txtwidth / 2;
  ...
  this.keydown = this.eventmanager.listen(
    "keydown",
    this.hit,
    self.eventhandler
  );
  // We'll quit mouse events if we get touch events
  this.mousedownID = this.eventmanager.listen(
    "mousedown",
    this.hit,
    self.eventhandler
  );
...
function slider(
  ...
  this.eventhandler = function(x, y, e) {
    ...
    if (e.type == "keydown") {
      ...
      if (e.keyCode == 36) {
        ...
        // <Page Up> key
        self.value = checkValue(
          self.value + 2 * self.step,
          self.min,
          self.max,
          self.step
        );
        ...
        if (e.keyCode == 37 || e.keyCode == 39) {
          // <left> or <right>
          self.value = checkValue(
            self.value + (e.keyCode - 38) * self.step,
            self.min,
            self.max,
            self.step
          );
          ...
          // <up> or <down>
          self.value = checkValue(
            self.value - (e.keyCode - 39) * self.step,
            self.min,
            self.max,
            self.step
          );
      ...
      if (self.DOMMouseScrollID) {
        self.eventmanager.quitlistening(
          "DOMMouseScroll",
          self.DOMMouseScrollID
        );
      ...
      if (e.type == "touchstart") {
        if (self.touchmoveID == null) {
          self.touchmoveID = self.eventmanager.listen(
            "touchmove",
            self.hit,
            self.eventhandler
          );
        ...
        if (self.touchendID == null) {
          self.touchendID = self.eventmanager.listen(
            "touchend",
            self.hit,
            self.eventhandler
          );
      ...
      self.value = checkValue(
        self.value - wheelDirection(e) * self.step,
        self.min,
        self.max,
        self.step
      );
      ...
      if (!self.mousemoveID) {
        self.mousemoveID = self.eventmanager.listen(
          "mousemove",
          self.hit,
          self.eventhandler
        );
    ...
    if (self.orientation === SLIDER_HORIZONTAL) {
      self.value = checkValue(
        self.min +
          self.range *
            (x - (self.x + self.sliderWidth / 2)) /
            (self.length - self.sliderWidth),
        self.min,
        self.max,
        self.step
      );
  ...
  this.valueToPos = function() {
    ...
    if (this.orientation === SLIDER_HORIZONTAL) {
      ...
      return (
        (this.length - this.sliderWidth) *
          ((this.max - this.value) / (this.max - this.min)) +
        this.sliderWidth / 2
      );
    }
  };
  ...
  this.draw = function() {
    ...
    for (var ctr = 0; ctr <= this.max - this.min; ctr++) {
      var stepxsize =
        (this.length - this.sliderWidth) / ((this.max - this.min) / this.step);
      this.ctx.beginPath();
      this.ctx.arc(
        stepxsize * ctr + this.sliderWidth / 2,
        this.width / 2,
        0.5,
        0,
        2 * Math.PI,
  ...
  this.mousewheel = this.eventmanager.listen(
    "mousewheel",
    this.hit,
    self.eventhandler
  );
  this.DOMMouseScrollID = this.eventmanager.listen(
    "DOMMouseScroll",
    this.hit,
    self.eventhandler
  );
  // The next one turns off the preceeding mouse ones if we receive it
  this.touchstartID = this.eventmanager.listen(
    "touchstart",
    this.hit,
    self.eventhandler
  );
        ...
        if (
          ...
          alert(
            "duplicate! ctr: " +
              ...
              queue[ctr].eventType +
              " x: " +
              queue[ctr].boundingbox.x +
              " y: " +
              queue[ctr].boundingbox.y +
              " xextent: " +
              queue[ctr].boundingbox.xextent +
              " yextent: " +
              queue[ctr].boundingbox.yextent
          );
          return queue[ctr].id;
        }
      }
